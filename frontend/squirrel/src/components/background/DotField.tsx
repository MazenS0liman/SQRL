...
const DotField = memo(({
  ...
  useEffect(() => {
    ...
    // Watch the parent element's actual box size directly — this catches
    // ANY layout-driven size change (sidebar hover expand/collapse, flex
    // reflow, container queries, etc.), not just window resizes. A plain
    // `window.resize` listener misses these because the viewport itself
    // never changes size when the sidebar toggles, only the flex layout
    // inside it does — which used to leave stale, uncovered canvas space
    // after the content column resized.
